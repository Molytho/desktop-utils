...
namespace detail {
    ...
    public:
        ...
        constexpr Source &get_source() noexcept { return m_source; }

        constexpr Callback &get_callback() noexcept { return m_callback; }
    };
} // namespace detail
...
public:
    ...
    void remove_item(detail::loop_element *wrapper) {
        erase_if(m_loop_items, [&wrapper](const auto &element) { return element.get() == wrapper; });
    }
