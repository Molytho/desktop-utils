...
public:
    explicit constexpr output(uint32_t id, std::string name) :
    ...
    constexpr output(output &&other) noexcept :
            m_swaybg_pid {other.m_swaybg_pid}, m_id {other.m_id}, m_picture {other.m_picture},
    ...
    [[nodiscard]] constexpr const pid_t &child_pid() const noexcept { return m_swaybg_pid; }

    [[nodiscard]] constexpr const uint32_t &id() const noexcept { return m_id; }
...
public:
    ...
    explicit background_manager(picture_manager pictures) :
            m_picture_manager {std::move(pictures)} {}
    ...
    [[nodiscard]] constexpr std::span<const output> outputs() const noexcept { return m_outputs; }
};
