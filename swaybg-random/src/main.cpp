...
int main(int argc, char *argv[]) {
    ...
    output_manager.add_added_listener(
        [&](const auto &output) { output->set_listeners(&output_listener, &background_manager); });
    output_manager.add_removed_listener(
        [&](const auto &output) { background_manager.remove_output(output->get_id()); });
